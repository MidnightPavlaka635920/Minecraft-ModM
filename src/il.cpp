...
void install_local(std::string& path_destination, std::string& path_tif, std::string& name, std::string& version ,std::string& loader){
    ...
    std::cout << "Installing...\n";
    std::string dest = path_destination + "/";
    try{
        fs::copy(path_tif, path_destination);
